...
import { BriefingJsonLd } from "@/components/seo/json-ld";
...
export async function generateMetadata({ params }: ArchiveDatePageProps): Promise<Metadata> {
  ...
  return {
    title: `${dateInfo.formattedKorean} 뉴스 브리핑`,
    description: `${dateInfo.formattedKorean} ${dateInfo.dayOfWeek} 뉴스 브리핑입니다. 정치, 경제, 사회 ${totalNews}개 뉴스 AI 요약.`,
    openGraph: {
      title: `${dateInfo.formattedKorean} 뉴스 브리핑 | Daily Wrap`,
      description: `${dateInfo.formattedKorean} ${dateInfo.dayOfWeek} 뉴스 브리핑입니다. 정치, 경제, 사회 ${totalNews}개 뉴스 AI 요약.`,
    },
  };
}
    ...
    <>
      ...
      <main className="min-h-screen relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 md:px-8 py-8 md:py-16">
          ...
          {/* 네비게이션 - 터치 타겟 44px 보장 */}
          <nav className="flex items-center justify-between mb-4 sm:mb-6 glass-card rounded-xl px-3 sm:px-4 py-2" aria-label="브리핑 네비게이션">
            <div className="min-w-[100px]">
              {prev ? (
                ...
                <span className="text-text-muted text-xs sm:text-sm px-2" aria-label="이전 브리핑 없음">
          ...
          {/* 뉴스 섹션들 */}
          <section className="space-y-6 sm:space-y-8" aria-label="뉴스 브리핑">
            <NewsSection
              section="politics"
