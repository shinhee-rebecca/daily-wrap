...
import { WebsiteJsonLd, BriefingJsonLd, ServiceFaqJsonLd } from "@/components/seo/json-ld";
...
export default async function HomePage() {
  ...
  return (
    <>
      {/* SEO: JSON-LD 구조화 데이터 */}
      <WebsiteJsonLd />
      <BriefingJsonLd briefing={briefing} dateFormatted={dateInfo.formattedKorean} />
      <ServiceFaqJsonLd />
      ...
      <main className="min-h-screen relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 md:px-8 py-8 sm:py-12 md:py-20">
          ...
          {/* 네비게이션 바 */}
          <nav className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0 mb-8 sm:mb-10 md:mb-12 glass-card rounded-xl px-4 sm:px-6 py-3 sm:py-4" aria-label="섹션 네비게이션">
            <ul className="flex items-center gap-4 sm:gap-6 flex-wrap list-none m-0 p-0" aria-label="뉴스 카테고리">
              <li className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-politics animate-pulse" aria-hidden="true" />
                <Caption tone="muted" size="sm">POLITICS</Caption>
              </li>
              <li className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-economy animate-pulse" style={{ animationDelay: "0.3s" }} aria-hidden="true" />
                <Caption tone="muted" size="sm">ECONOMY</Caption>
              </li>
              <li className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-society animate-pulse" style={{ animationDelay: "0.6s" }} aria-hidden="true" />
          ...
          {/* 뉴스 섹션들 */}
          <section className="space-y-6 sm:space-y-8" aria-label="오늘의 뉴스 브리핑">
            <NewsSection
              section="politics"
          ...
          {/* 푸터 */}
          <footer className="text-center py-8 sm:py-10 md:py-12 mt-12 sm:mt-14 md:mt-16" role="contentinfo">
