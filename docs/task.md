...
- [x] **베타 신청 CTA 컴포넌트**
  ...
  - 작업:
    ...
    - 이메일/카카오 ID 입력 폼 (제출은 console.log)
...
- [x] **요약 프롬프트 설계**
  ...
  - 작업: 한줄 헤드라인 + 4-5문장 요약 생성 (원문 복사 금지)
...
- [x] **데이터 페칭**
  - 파일: `lib/data/briefings.ts` (신규)
  - 작업: Server Component에서 Supabase 데이터 조회, Mock fallback
...
### 4.2 아카이브 페이지
- [x] **아카이브 목록**
  - 파일: `app/archive/page.tsx` (수정)
  - 작업: Supabase에서 최근 30일 브리핑 목록 조회, Mock fallback
  - 완료조건: 모든 가용 브리핑 표시
  - 테스트: 목록 렌더링 확인

- [x] **날짜별 브리핑 페이지**
  - 파일: `app/archive/[date]/page.tsx` (수정)
  - 작업: YYYY-MM-DD 형식 URL, 이전/다음 네비게이션, Supabase 데이터
  - 완료조건: 과거 브리핑 접근 가능
  - 테스트: `/archive/2026-01-17` 접근 확인

- [x] **아카이브 네비게이션**
  - 작업: 이전/다음 날 네비게이션, "오늘로 돌아가기" 버튼
  - 완료조건: 쉬운 날짜 간 이동
  - 테스트: 다양한 날짜 탐색

### 4.3 반응형 디자인
- [x] **Desktop 레이아웃 (1200px+)**
  - 작업: 최대 너비 컨테이너, 전체 타이포그래피 스케일
  - 테스트: 1440px 뷰포트에서 확인

- [x] **Tablet 레이아웃 (768px-1199px)**
  - 작업: 반응형 패딩/마진, 뉴스 카드 2단 그리드
  - 테스트: 768px, 1024px 뷰포트에서 확인

- [x] **Mobile 레이아웃 (<768px)**
  - 작업: 단일 컬럼, 터치 타겟 44x44px 이상, 축소된 타이포그래피
...
- [x] **Signup CTA 컴포넌트**
  - 파일: `components/beta/signup-cta.tsx`
  - 작업: 장식 테두리 박스 디자인, 프로그레스 바, 그라디언트 효과
  - 완료조건: 디자인 스펙 섹션 5.5와 일치
  - 테스트: 시각적 비교 완료

- [x] **신청 폼**
  - 파일: `components/beta/signup-cta.tsx` (CTA 컴포넌트에 통합)
  - 작업: 이메일 (필수), 카카오 ID (선택) 입력
  - 완료조건: 클라이언트 사이드 검증
  - 테스트: 유효/무효 데이터 제출 검증 동작

- [x] **폼 제출 연동**
  - 작업: `/api/beta-signup` 연결, 성공/에러 상태 표시
  - 완료조건: end-to-end 동작, 로딩/성공/에러 상태 UI
  - 테스트: 신청 완료 후 DB 저장 확인

- [x] **신청 카운터**
  - 작업: "XX/100 베타 테스터 모집 중" 실시간 표시
  - 완료조건: 정확한 카운트 표시, 프로그레스 바 애니메이션
  - 테스트: 신청 추가 후 카운터 갱신 확인

### 5.2 카카오톡 연동 준비
- [x] **오픈채팅방 설정 가이드**
  - 파일: `docs/kakaotalk-setup.md`
  - 작업: 오픈채팅방 생성 및 운영 방법 문서화
  - 완료조건: 명확한 가이드 문서
  - 테스트: 체크리스트 및 템플릿 포함

- [x] **브리핑 텍스트 내보내기**
  - 파일: `scripts/export-briefing.ts`
  - 작업: 카카오톡용 플레인 텍스트 생성 (날짜 + 섹션 + 헤드라인 + 요약 + 링크)
  - 스크립트: `pnpm export-briefing [날짜]`, `--compact` 옵션 지원
  - 완료조건: 가독성 좋은 텍스트 출력
  - 테스트: DB/Mock 데이터 지원, 간략/전체 형식 출력
